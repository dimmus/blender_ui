...
# -----------------------------------------------------------------------------
# Macro for configuring meson

# Meson arguments, assigned to local variable to make it mutable.
MESON_CONFIG_ARGS := $(BUILD_MESON_ARGS)

MESON_CONFIG_ARGS:=-DWITH_COMPILER_CCACHE=YES $(MESON_CONFIG_ARGS)

MESON_CONFIG = meson setup $(BUILD_DIR) # $(MESON_CONFIG_ARGS)
...
check_spelling_c: .FORCE
	@PYTHONIOENCODING=utf_8 $(PYTHON) \
	    ...
	    "$(BLENDER_DIR)/src" \
	    "$(BLENDER_DIR)/src/lib/guardedalloc" \
	    "$(BLENDER_DIR)/src/lib/ghost"
